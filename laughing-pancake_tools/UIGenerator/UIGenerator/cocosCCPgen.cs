...
namespace UIGenerator
{
    public class cocosCCPgen
    {
        ...
        public void AnimationLoader(JObject obj)
        {
        ...
        public void widgetrootoader(JObject obj)
        {
            List<Widgetnode> Widgetnodes = new List<Widgetnode>();
            JObject animation = JObject.Parse(obj["widgetTree"].ToString());
            JArray actionlist = JArray.Parse(animation["children"].ToString());
            foreach (JObject itemObj in actionlist)
            {
                Widgetnode root = new Widgetnode();
                string classname = itemObj["classname"].ToString();
                root.classname = classname;
                JObject options = JObject.Parse(itemObj["options"].ToString());
                string name = options["name"].ToString();
                string stag = options["tag"].ToString();
                ...
                JArray Childlist = JArray.Parse(itemObj["children"].ToString());
                if (Childlist != null)
                {
                    widgetnodeLoader(root, Childlist);
                }
            }
            Widgetlist = Widgetnodes;
        }

        public void widgetnodeLoader(Widgetnode root, JArray obj)
        {
            List<Widgetnode> Widgetnodes = new List<Widgetnode>();
            List<string> issues = new List<string>();

            //JArray actionlist = JArray.Parse(obj["children"].ToString());
            foreach (JObject itemObj in obj)
            {
                ...
                if (Childlist != null)
                {
                    widgetnodeLoader(node, Childlist);
                }
            }
        }
